// `workers.ts` acts as the main entrypoint for the worker processes.
...
const entrypoint = (mode: string): Promise<void> => Promise.resolve()
...
// -------------------------------------
;(async() => entrypoint(process.env.WORKER_NAME || process.argv[2]))()
